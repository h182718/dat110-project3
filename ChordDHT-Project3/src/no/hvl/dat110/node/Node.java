...
public class Node extends UnicastRemoteObject implements ChordNodeInterface {
    ...
    private boolean multicastMessage(Message message) throws AccessException, RemoteException {
        ...
        this.quorum = activenodesforfile.size() / 2 + 1;

        List<Message> nodes = new ArrayList<>(this.activenodesforfile);
        ...
        for (int i = 0; i < quorum; i++) {
            Message msg = nodes.get(i);
            String s = msg.getNodeID().toString();
            try {
                Registry reg = Util.locateRegistry(msg.getNodeIP());
                ChordNodeInterface node = (ChordNodeInterface) reg.lookup(s);
    ...
    public boolean majorityAcknowledged() throws RemoteException {
        ...
        return queueACK.stream().filter(Message::isAcknowledged).count() >= quorum;
    }
    ...
    public void onReceivedUpdateOperation(Message message) throws RemoteException {
        Operations oper = new Operations(this, message, activenodesforfile);
    ...
    public void multicastUpdateOrReadReleaseLockOperation(Message message) throws RemoteException {
        ...
        if (message.getOptype().equals(OperationType.READ)) {
